...
import com.pedro.repository.LivroRepository;
...
public class LivroController {
	...
	public ResponseEntity<Livro> listarPorId(@PathVariable Long id) {
		...
		if (livroOPT.isPresent()) {
			Livro livroDB = livroOPT.get();
			return ResponseEntity.ok(livroDB);
		}
	...
	public ResponseEntity<Livro> atualizar(@Valid @RequestBody Livro livro, @PathVariable Long id) {
		...
		livroDB.setQtdPaginas(livro.getQtdPaginas());
		livroDB.setPublicacao(livro.getPublicacao());
		return ResponseEntity.ok(livroDB);
	}
